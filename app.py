...
def inject_user_profiles():
    """Provide `sponsor` and `influencer` variables to all templates safely.
    This prevents Jinja `UndefinedError` when templates check for these values.
    """
    sponsor = None
    influencer = None
    try:
        if current_user and current_user.is_authenticated:
            if getattr(current_user, 'role', None) == 'sponsor':
                sponsor = Sponsor.query.filter_by(user_id=current_user.id).first()
            elif getattr(current_user, 'role', None) == 'influencer':
                influencer = Influencer.query.filter_by(user_id=current_user.id).first()
    except Exception:
        # If something goes wrong (e.g., db access during app startup), return None values
        sponsor = None
        influencer = None
    return dict(sponsor=sponsor, influencer=influencer)
...
def logout():
    logout_user()
    # Clear any previous flashed messages so the login page shows only the logout message
    try:
...
def send_ad_request(campaign_id):
    # Use the logged-in user as the influencer to prevent spoofing
    if current_user.role != 'influencer':
    ...
    # Prevent duplicate requests for the same campaign by the same influencer
    existing = AdRequest.query.filter_by(influencer_id=influencer_id, campaign_id=campaign_id).first()
    if existing:
...
def search_campaigns():
    ...
    campaigns = query.all()
    # Determine which campaigns the current influencer already requested
    requested_campaign_ids = set()
    try:
        if current_user.is_authenticated and current_user.role == 'influencer':
            existing = AdRequest.query.filter_by(influencer_id=current_user.id).all()
            requested_campaign_ids = {r.campaign_id for r in existing}
    except Exception:
        requested_campaign_ids = set()

    return render_template('search_campaigns.html', campaigns=campaigns, requested_campaign_ids=requested_campaign_ids)
